...
struct ContentView: View {
    ...
    private func addNewWord() {
        ...
        guard isShort(word: answer) else {
            showWordError(title: "Word is too short", message: "Please input word length higher then 3")
            return
        }
        
        guard isSameWord(word: answer) else {
            showWordError(title: "Word is same as root word", message: "Please try different combinations.")
            return
        }
    ...
    private func isSameWord(word: String) -> Bool {
        word != rootWord
    }
